...
function MeanRewardBeliefMDP(pomdp::P, resampler, max_frac_replaced) where {P<:POMDP}
    S = statetype(pomdp)
    b0 = resample(resampler, initialstate_distribution(pomdp), MersenneTwister(14))
    MeanRewardBeliefMDP{P, typeof(resampler), S, typeof(b0), actiontype(pomdp)}(
        pomdp,
        resampler,
        max_frac_replaced,
        S[],
...
function generate_sr(bmdp::MeanRewardBeliefMDP{P,R,S,B,A}, b::AbstractParticleBelief{S}, a, rng::AbstractRNG) where {P,R,S,B,A}
    ...
    if all_terminal || ws < eps(1.0/length(wm))
        ...
        return resample(bmdp.resample, initialstate_distribution(bmdp.pomdp), rng), 0.0
    end

    pc = resample(bmdp.resample, WeightedParticleBelief{statetype(bmdp.pomdp)}(pm, wm, ws, nothing), rng)
    ps = particles(pc)
...
function initialstate(bmdp::MeanRewardBeliefMDP, rng::AbstractRNG)
...
isterminal_act(bmdp::MeanRewardBeliefMDP, a) = isterminal_act(bmdp.pomdp, a)
