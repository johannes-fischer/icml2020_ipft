...
@add_arg_table s begin
    ...
    "-N"
        ...
        default = 1000
    "--test"
        help = "flag for using only one random solver and N=1"
        action = :store_true
    "--solver"
...
pomdp = ContinuousLightDark(big_step=parsed_args["step"],
                            transition_std=parsed_args["trans"],
                            observation_std_base=parsed_args["obs"])
...
for (k, solver) in solvers
    ...
    for i in 1:N
        ...
        N_p = 10000
        # up = SIRParticleFilter(pomdp, N_p, MersenneTwister(seed+i+90_000))
